...
        const isUserAdmin = ADMIN_UIDS.includes(user.uid); // <-- NEW

        // Check if the signed-in user's UID is in the admin list
        if (isUserAdmin && adminButton) { // <-- MODIFIED (uses the new variable)
            console.log("User is an admin!");
            adminButton.classList.remove('hidden');
        }

        // 2. Call our new function to lock or unlock controls
        setAdminControls(isUserAdmin); // <-- NEW
        // --- END NEW LOGIC ---
...
        .catch(error => {
            console.error("Firebase sign-in error:", error);s
...
s   const itemIndex = acceptedUpgrades.findIndex(item => item.resId === resIdToComplete);
    if (itemIndex > -1) {
...
s               showError({ message: "Could not save upgrade to cloud." });
            });
        displayAcceptedUpgrades();
        displayCompletedUpgrades();
    }
...
s                     <div class="rec-upgrade-to">Upgrade To<br><strong>${rec.upgradeTo}</strong></div>
...
S   const originalTargetRooms = rules.targetRooms.toUpperCase().split(',').map(r => r.trim()).filter(Boolean);
...
s                   const potentialUpgradeRoom = roomHierarchy[i];
                    const potentialBedType = getBedType(potentialUpgradeRoom);
s                   if (originalBedType !== potentialBedType || ineligibleUpgrades.includes(potentialUpgradeRoom)) continue;
...
                          s upgradeTo: potentialUpgradeRoom, score: score,
...
            arrival: arrival,
            departure: departure,
s           status: values[statusIndex] ? values[statusIndex].trim().toUpperCase() : '',
...
An       { roomNumber: '202', code: 'DK-K' }, { roomNumber: '204', code: 'DK-K' }, { roomNumber: '205', code: 'DK-K' },
...
Example:       { roomNumber: '108', code: 'TK-K' }, { roomNumber: '208', code: 'TK-K' }, { roomNumber: '209', code: 'TK-K' },
...
s       { roomNumber: '103', code: 'TQ-QQ' }, { roomNumber: '314', code: 'TQ-QQ' }, { roomNumber: '203', code: 'TQ-QQ' },

...
async function handleAutoLoad() {
    ...
    // 2. Construct the exact ID we expect to find
    ...
    try {
        // 3. FETCH EXACT DOCUMENT (Fast & Cheap)
        ...
        setTimeout(() => {
            try {
                ...
                alert(`Success! Loaded ${realFileName}`);
            } catch (err) {
                showError(err);
            } finally {
                btn.disabled = false;
...
function getBedType(roomCode) {
    ...
    // --- NEW LOGIC FOR ONE BED TYPES ---
    if (roomCode === 'DBDBADA-DD') return 'QQ';

    // --- NEW LOGIC FOR ICH BED TYPES ---

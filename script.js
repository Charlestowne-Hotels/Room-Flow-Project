...
// ... (Your MASTER_INVENTORIES object sits above this) ...
...
let originalAllReservations = []; // Stores the raw baseline for matrix comparison
...
function handleRefresh() {
    if (currentCsvContent) {
        ...
        setTimeout(() => {
            try {
                // If we have accepted upgrades, maintain the "Post-Acceptance" view
                if (acceptedUpgrades.length > 0) {
                    ...
                    // Otherwise, show standard scenario generation
...
function displayMatrixOnlyView(results) {
    ...
    hierarchy.forEach(roomCode => {
        ...
        dates.forEach((date, i) => {
             ...
             let val = 0;
             // Safe lookup in the stored simulation result
...
function displayResults(data) {
    ...
    displayAcceptedUpgrades();
    // Removed projected matrix from initial view as requested
...
function renderScenarioContent(name, scenarioData, parent, currentMatrix) {
    ...
    // --- CALC MATRICES ---
    // 1. Current (Base) Matrix - from parameter (Calculated in displayResults)
    ...
    // NOTE: We are NOT displaying the Projected Matrix here anymore as requested.
    // Only the Current Availability (Base) is shown to help decision making.
    
    // --- RENDER MATRICES ---
    const matrixContainer = document.createElement('div');
    matrixContainer.innerHTML = generateMatrixHTML("Current Availability (Base)", currentRows, headers, currColTotals);

...
// ==========================================
//    SETTINGS & TOGGLE UI LOGIC (Updated)
// ==========================================

// [NEW] Injects the toggle HTML into the settings modal
function injectViewModeToggle() {
    const container = document.getElementById('admin-rules-container'); 
    if (!container) return;
    
    // Prevent duplicate injection
    if (document.getElementById('view-mode-wrapper')) return;

    const wrapper = document.createElement('div');
    wrapper.id = 'view-mode-wrapper';
    wrapper.style.cssText = 'margin-bottom: 15px; padding: 10px; background: #f1f3f5; border-radius: 5px; display: flex; align-items: center; justify-content: space-between;';

    wrapper.innerHTML = `
        <label for="view-mode-toggle" style="font-weight:bold; color:#333; cursor:pointer;">
            Enable Individual Upgrade List Mode
            <div style="font-weight:normal; font-size:12px; color:#666;">Switch from Scenarios (Paths) to a raw list of all potential upgrades.</div>
        </label>
        <div style="position: relative; display: inline-block; width: 50px; height: 24px;">
            <input type="checkbox" id="view-mode-toggle" style="opacity: 0; width: 0; height: 0;">
        ...
        <style>
            input:checked + .slider { background-color: #4343FF; }
            input:checked + .slider .slider-before { transform: translateX(26px); }
    ...
    const saveBtn = document.getElementById('save-rules-btn');
    if(saveBtn) container.insertBefore(wrapper, saveBtn);
}

// [REPLACES EXISTING] Updates form values and sets the toggle state
function updateRulesForm(profileName) {
    const profile = profiles[profileName];
    if (!profile) return;
    document.getElementById('hierarchy').value = profile.hierarchy;
    ...
    // Set toggle state based on saved profile preference
    const viewToggle = document.getElementById('view-mode-toggle');
    if (viewToggle) {
        viewToggle.checked = profile.viewMode === 'individual';
    }

    populateOooDropdown(); 
}

// [REPLACES EXISTING] Saves rules including the new viewMode preference
async function handleSaveRules() {
    const currentProfile = document.getElementById('profile-dropdown').value;
    const btn = document.getElementById('save-rules-btn');
    const status = document.getElementById('save-status');
    ...
    const viewToggle = document.getElementById('view-mode-toggle');
    const viewMode = viewToggle && viewToggle.checked ? 'individual' : 'paths';
...
// ==========================================
//    CORE ALGORITHM LOGIC (Updated)
// ==========================================

// [REPLACES EXISTING] Main entry point for generation, routes based on View Mode
function generateScenariosFromData(allReservations, rules) {
    const masterInventory = getMasterInventory(rules.profile);
    if (!Object.keys(masterInventory).length) return { error: `No inventory for ${rules.profile}` };
    ...
    // --- CHECK VIEW MODE ---
    if (rules.viewMode === 'individual') {
        const individualCandidates = generateAllEligibleCandidates(activeReservations, masterInventory, rules, completedResIds);
        return { 
            scenarios: null, 
            individualCandidates: individualCandidates, 
            inventory: todayInventory, 
            matrixData, 
            message: `Generated ${individualCandidates.length} individual opportunities.` 
        };
    }
...
// [NEW] Generates a flat list of all candidates without complex path simulation
function generateAllEligibleCandidates(allReservations, masterInv, rules, completedIds) {
    const candidates = [];
    const startDate = parseDate(rules.selectedDate);
    const hierarchy = rules.hierarchy.toUpperCase().split(',').map(r => r.trim()).filter(Boolean);
    const ineligible = rules.ineligibleUpgrades.toUpperCase().split(',');
    ...
    for (let i = 0; i < 14; i++) {
        const d = new Date(startDate); d.setUTCDate(d.getUTCDate() + i);
        const dStr = d.toISOString().split('T')[0];
        baseInventory[dStr] = getInventoryForDate(masterInv, buildReservationsByDate(allReservations), d);
    }

    allReservations.forEach(res => {
        if (completedIds.has(res.resId) || res.status !== 'RESERVATION') return;
        
        // Only look at arrivals within next 7 days from selected date
        const diffTime = res.arrival - startDate;
        ...
        // Find all possible upgrades for this guest
        for (let u = currentIdx + 1; u < hierarchy.length; u++) {
            const targetRoom = hierarchy[u];
            if (ineligible.includes(targetRoom)) continue;
            if (getBedType(targetRoom) !== originalBed && originalBed !== 'OTHER') continue;

            // Check Availability for specific stay dates
            let available = true;
            let checkDate = new Date(res.arrival);
            while (checkDate < res.departure) {
                const dStr = checkDate.toISOString().split('T')[0];
                if (!baseInventory[dStr] || (baseInventory[dStr][targetRoom] || 0) <= 0) {
...
// ==========================================
//    DISPLAY & INTERACTION LOGIC (Updated)
// ==========================================

// [REPLACES EXISTING] Routes the display to either Scenarios or Individual List
function displayResults(data) {
    showLoader(false);
    if (data.error) { showError({ message: data.error }); return; }
    if (data.acceptedUpgrades) acceptedUpgrades = data.acceptedUpgrades;
    ...
    // ROUTER Logic
    if (data.individualCandidates) {
        displayIndividualCandidates(data.individualCandidates);
    } else {
        displayScenarios(currentScenarios); 
    }
...
// [NEW] Renders the flat list table
function displayIndividualCandidates(candidates) {
    const container = document.getElementById('recommendations-container');
    container.innerHTML = '';
    
    if (!candidates || candidates.length === 0) {
        container.innerHTML = '<p style="padding:20px; text-align:center;">No eligible upgrades found based on current availability.</p>';
        return;
    }

    const table = document.createElement('table');
    table.style.cssText = 'width:100%; border-collapse:collapse; margin-top:15px; background:white; font-size:13px;';
    
    const thead = `
        <thead>
            <tr style="background:#f8f9fa; border-bottom:2px solid #eee; text-align:left;">
                <th style="padding:12px;">Guest / Res ID</th>
                <th style="padding:12px;">Dates</th>
                <th style="padding:12px;">Current Room</th>
                <th style="padding:12px;">Upgrade To</th>
    ...
    candidates.forEach(cand => {
        ...
        const vipTag = cand.vipStatus ? `<span style="color:red; font-weight:bold; font-size:11px; margin-left:5px;">${cand.vipStatus}</span>` : '';

        tr.innerHTML = `
            <td style="padding:10px;"><strong>${cand.name}</strong><br><span style="color:#888;">${cand.resId}</span>${vipTag}</td>
            <td style="padding:10px;">${cand.arrivalDate.slice(0,5)} - ${cand.departureDate.slice(0,5)}</td>
            <td style="padding:10px;">${cand.room}</td>
            <td style="padding:10px; color:#4343FF; font-weight:bold;">${cand.upgradeTo}</td>
            <td style="padding:10px;">${cand.revenue}</td>
            <td style="padding:10px; text-align:right;">
                <button class="accept-single-btn" style="background:#28a745; color:white; border:none; padding:6px 12px; border-radius:4px; cursor:pointer;">Accept</button>
            </td>
        `;
        
        tr.querySelector('.accept-single-btn').addEventListener('click', () => {
            handleAcceptScenario("Individual Selection", [cand]);
        });
    ...
    title.innerHTML = `<h3>Individual Upgrade Candidates (${candidates.length})</h3><p style="color:#666; font-size:12px;">Note: Accepting an upgrade here recalculates availability and may invalidate others in this list.</p>`;
    
    container.appendChild(title);
    container.appendChild(table);
}

// [REPLACES EXISTING] Handles accepting both Scenarios (Paths) and Individual items
function handleAcceptScenario(scenarioName, manualData = null) {
    const scenario = manualData || currentScenarios[scenarioName];

    if (!scenario || !scenario.length) { alert("No upgrades available."); return; }
    
    const confirmMsg = manualData 
        ? `Accept upgrade for ${scenario[0].name} to ${scenario[0].upgradeTo}?`
        : `Accept all ${scenario.length} upgrades in ${scenarioName}?`;

    if(!confirm(confirmMsg)) return;

    acceptedUpgrades.push(...scenario);
    
    // Clear scenarios to force recalculation
    currentScenarios = {}; 
    ...
    setTimeout(() => {
        try {
            ...
            // If in individual mode, refresh the list. If in path mode, show the Matrix view.
            if (currentRules.viewMode === 'individual') {
                displayResults(results); 
            } else {
                displayMatrixOnlyView(results); 
            }
...
// Ensure you call injectViewModeToggle() inside your existing DOMContentLoaded event
// Example:
/*
document.addEventListener('DOMContentLoaded', function() {
    // ... existing initialization code ...
    injectViewModeToggle(); // <--- ADD THIS LINE
});
*/

...
// --- NEW HELPER FUNCTION TO MANAGE ADMIN CONTROLS ---
function setAdminControls(isAdmin) {
    ...
    formElementsToDisable.forEach(elementId => {
        const el = document.getElementById(elementId);
        if (el) {
            // Disable the element if the user is NOT an admin
            el.disabled = !isAdmin;
        }
    });

    // Optional: Add a visual cue for non-admins
    // You can add a class to a container to grey it out.
    // Example: const rulesContainer = document.querySelector('.rules-container');
    // if (rulesContainer) {
    //     if (isAdmin) {
    //         rulesContainer.classList.remove('non-admin-disabled');
    //     } else {
    //         rulesContainer.classList.add('non-admin-disabled');
    //     }
    // }
}
...
        if(adminButton) adminButton.classList.add('hidden');
        setAdminControls(false); // <-- UPDATED (Disable controls when logged out)
    }
});
...
DOM             showError({ message: "Could not save upgrade to cloud." });
            });
        displayAcceptedUpgrades();
        displayCompletedUpgrades();
    }
...
            _           Value of Reservation: <strong>${rec.revenue}</strong>
                    </div>
                </div>
                <div class="rec-actions">
...
Open   document.getElementById('loader').style.display = show ? 'block' : 'none';
...
F       let processingQueue = useDefaultLogic ? [...roomHierarchy] : [...originalTargetRooms];
        if (!useDefaultLogic) {
...
M     { roomNumber: '212', code: 'GMVT-QQ/POC' }, { roomNumber: '220', code: 'KBS-K/POC' }, { roomNumber: '319', code: 'KBS-K/POC' },
...
Click   const parts = dateStr.split(/[-\/]/);
    if (parts.length === 3) {
...
        dates.forEach(date => {
            const dateString = date.toISOString().split('T')[0];
  _         row.availability.push((totalInventory[roomCode] || 0) - (reservationsByDate[dateString]?.[roomCode] || 0));
        });
